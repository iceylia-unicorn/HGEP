...
def _apply_feats_type(data, feats_type: int):
    ...
    if feats_type in (0, 6, 7, -1, 8, 9):
        ...
        for i in range(len(features_list)):
            ...
            features_list[i] = torch.sparse.FloatTensor(idx, val, torch.Size([dim, dim])).to_dense()
    elif feats_type == 3:
        for i in range(len(features_list)):
            dim = features_list[i].shape[0]
            idx = np.vstack((np.arange(dim), np.arange(dim)))
...
def _summarize(records: list[RunRecord]):
    if len(records) == 0:
        return {}
...
def build_parser():
    ...
    ap.add_argument("--hgmp_ckpt", type=str, required=True)
    ap.add_argument("--typepair_ckpt", type=str, default=None)
    ap.add_argument("--hgprompt_ckpt", type=str, required=True)

    # shared legacy eval args
    ...
    # faithful-ish HGPrompt downstream args
...
def main():
    ...
    args.typepair_ckpt = args.typepair_ckpt or args.hgmp_ckpt
    ...
    for method in args.methods:
        ...
        for split_seed in args.seeds:
            for repeat_id in range(args.repeats):
                if method == "hgprompt":
                    record = run_hgprompt_once(args, args.hgprompt_ckpt, split_seed, repeat_id)
                elif method == "typepair":
                    record = run_legacy_method_once(args, "typepair", args.typepair_ckpt, split_seed, repeat_id)
                elif method == "hgmp":
                    record = run_legacy_method_once(args, "hgmp", args.hgmp_ckpt, split_seed, repeat_id)
                elif method == "hgmp_prompt":
                    record = run_legacy_method_once(args, "hgmp_prompt", args.hgmp_ckpt, split_seed, repeat_id)
                else:
    ...
    per_run_rows = [
        {
            ...
            "test_micro": r.test_micro,
            "test_macro": r.test_macro,
    ...
    for method in sorted(set(r.method for r in records)):
        for split_seed in sorted(set(r.split_seed for r in records if r.method == method)):
            subset = [r for r in records if r.method == method and r.split_seed == split_seed]
            stat = _summarize(subset)
    ...
    for method in sorted(set(r.method for r in records)):
        ...
        overall[method] = _summarize(subset)
