...
class PEPromptRelation(nn.Module):
    """Pure PE edge prompt used by PEPrompt downstream probing."""
    ...
    def forward(
        ...
        agg_dict = {ntype: torch.zeros_like(x) for ntype, x in x_dict.items()}
        deg_dict = {
        ...
        for ntype, x in x_dict.items():
            if self.aggr == "mean":
                agg = agg_dict[ntype] / deg_dict[ntype].clamp_min(1.0)
            else:
                agg = agg_dict[ntype]
            out[ntype] = self.ln[ntype](x + self.alpha * self.drop(agg))
        return out
...
class HGMPPEPromptHGNN(nn.Module):
    ...
    def forward(self, targetnode, x, edge_index=None, edge_feature_dict=None, homo_graph=None, graph=None):
        ...
        Args:
            ...
            graph: 保留该参数以兼容 HGT/GCN wrapper 调用；纯 PE prompt 不使用。
        """
        if self.hgnn_type == "HGT":
