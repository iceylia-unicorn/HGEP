...
def _fanout_to_list(fanout: Union[int, Sequence[int]], num_hops: int) -> List[int]:
    if isinstance(fanout, int):
        return [fanout] * num_hops
    fanout = list(fanout)
    if len(fanout) != num_hops:
...
def build_hetero_neighbor_loader(
    ...
    num_hops: int,
    fanout: Union[int, Sequence[int]] = (25, 15),
    seed_nodes: Optional[torch.Tensor] = None,
    shuffle: bool = True,
    num_workers: int = 0,
    persistent_workers: bool = False,
    disjoint: bool = False,
    ...
    Args:
        data: full hetero graph
        input_ntype: seed node type
        batch_size: number of seed nodes per mini-batch
        num_hops: sampling hops
        fanout: per-hop neighbors
        seed_nodes: optional subset of input_ntype node ids
        disjoint: if True, keep each seed-induced subgraph disjoint in a batch
    """
    fanout_list = _fanout_to_list(fanout, num_hops)
    num_neighbors: Dict[Tuple[str, str, str], List[int]] = {
        etype: fanout_list for etype in data.edge_types
    }

    if seed_nodes is None:
        seed_nodes = torch.arange(data[input_ntype].num_nodes, dtype=torch.long)
    else:
        seed_nodes = seed_nodes.long().cpu()

    loader = NeighborLoader(
        data,
        input_nodes=(input_ntype, seed_nodes),
